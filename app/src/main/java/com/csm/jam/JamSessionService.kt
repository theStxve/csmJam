...
class JamSessionService : Service() {
    ...
    companion object {
        ...
        // Set by onCreate/onDestroy so callers can check without binding
        @Volatile var isRunning: Boolean = false
        // Direct reference to running instance so updateTrack can call updateNotification
        // without risking a startForegroundService-from-background crash on Android 12+
        @Volatile private var instance: JamSessionService? = null
        ...
        /**
         * Safe notification update that avoids the Android 12+ ForegroundServiceStartNotAllowedException.
         * If the service is already running, update its notification directly via the instance reference.
         * Only falls back to startForegroundService when the service is not yet running.
         */
        fun updateTrack(context: Context, role: String, trackName: String?, isPlaying: Boolean = false) {
            val svc = instance
            if (isRunning && svc != null) {
                // Service is alive — update notification directly without going through startForegroundService
                try {
                    svc.updateNotification(role, trackName, isPlaying)
                } catch (_: Exception) {}
            } else {
                // Service not running yet — it is safe to start it from foreground
    ...
    override fun onCreate() {
        ...
        // Acquire WakeLock and Low-Latency WifiLock for continuous sync even with screen off
        try {
            val powerManager = getSystemService(Context.POWER_SERVICE) as? PowerManager
            wakeLock = powerManager?.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "csm_jam:session_wakelock")?.apply {
        ...
        try {
            ...
            wifiLock = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                wifiManager?.createWifiLock(WifiManager.WIFI_MODE_FULL_LOW_LATENCY, "csm_jam:session_wifilock")
            } else {
                @Suppress("DEPRECATION")
                wifiManager?.createWifiLock(WifiManager.WIFI_MODE_FULL_HIGH_PERF, "csm_jam:session_wifilock")
            }?.apply {
                setReferenceCounted(false)
                acquire()
            }
